"""
Jina Embeddings v4 - Jina-compatible API Server (with base64 support)

For LightRAG with EMBEDDING_BINDING=jina
"""
...
class EmbeddingRequest(BaseModel):
    ...
    embedding_type: Optional[str] = "float"  # "float" or "base64"


class EmbeddingData(BaseModel):
    object: str = "embedding"
    embedding: str | list[float]  # base64 string or list[float]
    index: int
...
app = FastAPI(title="Jina Embeddings v4 API (Jina-compatible)", lifespan=lifespan)
...
async def create_embeddings(request: EmbeddingRequest, authorized: bool = Depends(verify_api_key)):
    ...
    # Output format
    use_base64 = request.embedding_type == "base64"
    ...
    try:
        ...
        logger.info(f"Encoded {len(texts)} texts | task={task} | dim={dimensions} | base64={use_base64} | {elapsed:.3f}s")
        
        # Forming a response
        data = []
        for i, emb in enumerate(embeddings):
            if use_base64:
                # Convert to float32 and then to base64
                emb_bytes = emb.astype(np.float32).tobytes()
                emb_b64 = base64.b64encode(emb_bytes).decode('ascii')
                data.append(EmbeddingData(embedding=emb_b64, index=i))
            else:
...
@app.get("/")
async def root():
    return {"service": "Jina Embeddings v4 API (Jina-compatible)", "base64_support": True, "docs": "/docs"}
